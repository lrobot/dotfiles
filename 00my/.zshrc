#set -x
NEWLINE=$'\n'
#export PS1="$fg[cyan]%}$USER@%{$fg[blue]%}%m %d %F{green}${SSH_TTY:+%n@%m}%f%B${SSH_TTY:+:}%b%F{blue}%B%1~%b%F{yellow}%B%(1j.*.)%(?..!)%b%f %B${SUFFIX}%b ${NEWLINE}"
...
function () {
  ...
  #export PS1="$fg[cyan]%}$USER@%{$fg[blue]%}%m %d %F{green}${SSH_TTY:+%n@%m}%f%B${SSH_TTY:+:}%b%F{blue}%B%1~%b%F{yellow}%B%(1j.*.)%(?..!)%b%f %B${SUFFIX}%b"
  if [[ -n "$TMUXING" ]]; then
...
function -report-start-time() {
  ...
  if [ $ZSH_START_TIME ]; then
    ...
    export RPROMPT="%F{cyan}%{$__WINCENT[ITALIC_ON]%}${ELAPSED}%{$__WINCENT[ITALIC_OFF]%}%f $RPROMPT_BASE ${NEWLINE}"
    unset ZSH_START_TIME
  else
    export RPROMPT="$RPROMPT_BASE ${NEWLINE}"
  fi
}
